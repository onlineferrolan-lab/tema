  ...
  options: {
   ...
   // @param bool noclone (optional) true only if item should not be cloned on the other list
    _setSelected: function(item, selected, noclone) {
        ...
        // On more recent versions of jQuery UI, the ui-helper-hidden span now contains the LI's label, so we have to
        // handle that specific case to keep this function working
        if (item.prop('tagName') == 'SPAN' && item.hasClass('ui-helper-hidden')) {
            item = item.parent();
        }
        var optionLink = item.data('multiselect.optionLink');
