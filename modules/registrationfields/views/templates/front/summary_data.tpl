	...
	<form id="update_registration_form" class="std" action="{$link->getModuleLink('registrationfields', 'myinfo')|escape:'htmlall':'UTF-8'}" name="update_registration" method="POST" enctype="multipart/form-data">
        ...
        data-v="{$field['dependant_value']|escape:'htmlall':'UTF-8'}">
			...
			<div class="clearfix form-group margin-form"></div>
				...
				<div class="col-lg-9">
					{if $version >= 1.6}
						<button id="updateFields"{$non_editable|escape:'htmlall':'UTF-8'} class="btn btn-default button button-medium btn-primary" name="updateFields" type="submit"><span>{l s='Save' mod='registrationfields'}<i class="icon-chevron-right right"></i></span></button>
					{else}
						<p class="submit">
							<input id="updateFields"{$non_editable|escape:'htmlall':'UTF-8'} type="submit" value="{l s='Save' mod='registrationfields'}" name="updateFields" class="button btn btn-primary">
						</p>
					{/if}
				</div>
			</div>
