...
{foreach $available_filters as $subtitle => $filters}
	...
	<div class="filter-group">
		...
		<div class="filter-items-list">
			{foreach $filters as $key => $filter}<a href="#" class="filter-group-item{if isset($blocked[$key])} blocked{/if}" data-key="{$key|escape:'html':'UTF-8'}"{if !empty($filter.warning)} data-warning="{$filter.warning|escape:'html':'UTF-8'}{/if}">
