...
* busqueda avanzada
* */
if($('body').hasClass('category-id-11699')){
...
var af = {
				...
				af.$selectedFilters.on('click', 'a', function(e) {
                   ...
                   var $parentRow = $(this).parent(),
					...
					if ($(this).hasClass('close')) {
						//mod ecomm
                        if ($groupBlock.hasClass('type-1') || $groupBlock.hasClass('type-2') || $groupBlock.hasClass('type-6')) {
            ...
            toggleableContent: function() {
                af.$filterBlock.on('click', '.af_subtitle.toggle-content', function(e) {
                    ...
                    if (!$filter.hasClass(af.noItemsClass)) {
                        ...
                        // 🧠 Si el body tiene la clase .amazingfilterlaterlactive → cerrar los demás al abrir uno
                        if (!$('body').hasClass('amazingfilterlaterlactive')) {
                            if ($filter.hasClass('closed')) {
                                $filter.siblings('.af_filter').addClass('closed');
                            }
                            $filter.toggleClass('closed');
                        }
                        // 🕹️ Si no tiene la clase, comportamiento original
                        else {
                            $filter.toggleClass('closed');
		...
		theme: {
			...
			pagination: function() {
				$(document).on('click','.'+pagination_class+' a', function(e) {
                    ...
                    //mod ecomm, sito, sinos no funciona paginacion en el seopage el modulo
                    if ($("body").attr('id') == "module-af_seopages-seopage"){
                        var urlfinal = prestashop.urls.current_url+"?page="+page;
	...
	updateListAfter: function(jsonData) {
		...
		//mod ecomm, si estamos viendo subcategorias, las ocultamos
		$('.subcategories').addClass('hidden');

		//mod ecomm
		$('body').addClass('showproductlistadvancedsearch');
	...
	updateFilteringBlocks: function(jsonData) {
		...
		$('.af_filter').each(function() {
			...
			if ($.inArray($(this).data('type'), [1,2,5,6]) > -1) { // checkbox, radio, textbox
