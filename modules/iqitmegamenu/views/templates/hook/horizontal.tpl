...
*}
		...
		<div id="iqitmegamenu-horizontal" class="iqitmegamenu  cbp-nosticky {if $menu_settings.hor_s_transparent && $menu_settings.hor_sticky} cbp-sticky-transparent{/if}" role="navigation">
			<div class="container">
				...
				<nav id="cbp-hrmenu" class="cbp-hrmenu cbp-horizontal cbp-hrsub-narrow  {if $menu_settings.hor_animation==1}cbp-fade{/if} {if $menu_settings.hor_animation==2}cbp-fade-slide-bottom{/if} {if $menu_settings.hor_animation==3}cbp-fade-slide-top{/if} {if $menu_settings.hor_s_arrow}cbp-arrowed{/if} {if !$menu_settings.hor_arrow} cbp-submenu-notarrowed{/if} {if !$menu_settings.hor_arrow} cbp-submenu-notarrowed{/if} {if $menu_settings.hor_center} cbp-menu-centered{/if} ">
					{if Configuration::get('iqitmegamenu_show_mobile_menu_in_desktop') == 1}
    					<div id="show_mobile_menu_in_desktop">
                            <span class="menu_bar"></span>
                            <span class="text_menu">{l s='Productos' d='Shop.Theme.Product'}</span>
                        </div> 
                    {/if}
					<ul>
    ...
    {if $tab.url_type == 2}<a role="button" class="cbp-empty-mlink" title="{$tab.title|replace:'/n':'<br />'}">{else}<a href="{$tab.url}" title="{$tab.title|replace:'/n':'<br />'}" {if $tab.new_window}target="_blank"{/if}>{/if}
...
{*										<span class="closetxt">*}
{*											{l s='Menú' module='iqitmegamenu'}*}
{*										</span>*}
										<i class="material-icons d-inline">close</i>
									</div>
								</li>
								{include file="module:iqitmegamenu/views/templates/hook/_partials/mobile_menu.tpl" menu=$mobile_menu type="top"}

								<li class="itemsbottomcontentsep"></li>
								{include file="module:iqitmegamenu/views/templates/hook/_partials/mobile_menu.tpl" menu=$mobile_menu_bottom type="bottom"}

								<li class="extrabottom">
									<div id="language_menu_content"></div>

									{widget name='ps_socialfollow' hook='footer'}
								</li>
							</ul>
						</div>
