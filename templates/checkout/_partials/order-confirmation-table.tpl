    ...
    {block name='order_confirmation_table'}
      {foreach from=$products item=product}
        <div class="order-line row">
          <div class="col-sm-4 col-xs-4 image_block">
            <span class="image">
              {if !empty($product.default_image)}
              
                <img src="{$product.default_image.bySize.home_default.url}" class="img-fluid" loading="lazy" width="{$product.default_image.bySize.home_default.width}" height="{$product.default_image.bySize.home_default.height}"/>
              {else}
          ...
          <div class="col-sm-8 col-xs-8 details">
            ...
            {/if}
              <span class="product_name-order">{$product.name}</span>
              
            {if $add_product_link}
                </a>
            {/if}
            <span class="unit_price_product"><strong>{l s='Precio unitario: ' d='Shop.Theme.Catalog'}</strong>{$product.price}</span>
            <span class="quantity_product"><strong>{l s='Cantidad: ' d='Shop.Theme.Catalog'}</strong>{$product.quantity}</span>
            <span class="total_price_product"><strong>{l s='Total: ' d='Shop.Theme.Catalog'}</strong>{$product.total}</span>    
